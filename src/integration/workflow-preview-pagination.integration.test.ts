...
import { CachifyService } from '../main/application/cachify-service'
...
class InMemoryCacheGateway implements CacheGateway {
  private readonly map = new Map<string, { value: string; ttlSeconds: number | null }>()
...
describe('workflow preview pagination integration', () => {
  it('returns next cursor and supports paging across preview requests', async () => {
    const tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'cachify-intg-'))
