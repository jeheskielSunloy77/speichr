...
const createDefaultRuleForm = (connection: ConnectionProfile): RuleFormState => ({
  name: '',
  metric: 'errorRate',
  threshold: '0.2',
  lookbackMinutes: '5',
...
export const AlertsPanel = ({ connection }: AlertsPanelProps) => {
  ...
  const rulesQuery = useQuery({
    ...
    queryFn: async () => unwrapResponse(await window.cachify.listAlertRules()),
  })
  ...
  const saveRuleMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingRuleId) {
        return unwrapResponse(
          await window.cachify.updateAlertRule({
            id: editingRuleId,
            rule,
          }),
        )
      ...
      return unwrapResponse(
        await window.cachify.createAlertRule({
          rule,
        }),
      )
    },
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : 'Unable to save alert rule.')
    },
  })
  ...
  const deleteRuleMutation = useMutation({
    mutationFn: async (id: string) =>
      unwrapResponse(
        await window.cachify.deleteAlertRule({
          id,
        }),
      ),
    onSuccess: async () => {
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : 'Unable to delete alert rule.')
    },
  })
