		...
		queryFn: async () =>
			unwrapResponse(
				await window.speichr.getKeyspaceActivity({
					connectionId: connection.id,
					from: toIsoOrFallback(activityFrom, new Date(Date.now() - ONE_HOUR_MS)),
		...
		<div className='grid min-h-0 gap-3'>
								...
								{dashboard?.timeline.slice(0, 80).map((event) => (
									<div key={event.id} className='space-y-1 border p-2 text-xs'>
										<div className='flex items-center justify-between gap-2'>
											...
											<Badge
												variant={event.status === 'error' ? 'destructive' : 'outline'}
											>
												{event.status}
											</Badge>
										</div>
						...
						<div className='grid gap-3 md:grid-cols-3'>
							...
							<div className='space-y-1.5'>
								...
								<Input
									...
									onChange={(event) => setActivityIntervalMinutes(event.target.value)}
								/>
							</div>
						</div>
									...
									<TableBody>
										{(compareQuery.data?.metrics ?? []).map((metric) => (
											<TableRow key={metric.metric}>
												...
												<TableCell>
													{metric.delta} (
													{metric.deltaPercent === null ? 'n/a' : `${metric.deltaPercent}%`})
												</TableCell>
												<TableCell>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Preview timeline/log/diagnostic/metric payloads and export with checksum
						and redaction profile metadata.
					</CardDescription>
				</CardHeader>
				<CardContent className='space-y-3'>
					<div className='grid gap-3 md:grid-cols-4'>
						...
						<div className='space-y-1.5'>
							...
							<select
								...
								onChange={(event) =>
									setIncidentRedactionProfile(event.target.value as 'default' | 'strict')
								}
							>
									...
									<TableBody>
										{incidentBundlesQuery.data?.map((bundle) => (
											<TableRow key={bundle.id}>
												...
												<TableCell>
													{bundle.timelineCount}/{bundle.logCount}/{bundle.diagnosticCount}/
													{bundle.metricCount}
												</TableCell>
